...
export const AudioRecorder: React.FC<AudioRecorderProps> = ({ 
    ...
    return (
      <div className="w-full flex flex-col items-center gap-8 animate-fade-in z-10">
        <div className="text-center space-y-3 mt-4">
          <h2 className="text-2xl font-heading font-bold text-brand-charcoal dark:text-dark-text transition-colors">{t('rec_recording')}</h2>
        ...
        <div className="w-full px-4">
            <Button onClick={handleToggle} variant="danger" fullWidth className="h-16 text-lg rounded-2xl shadow-glow-coral">
              <span className="mr-3 text-2xl">■</span> {t('rec_stop')}
            </Button>
        </div>
      </div>
    );
  ...
  return (
    <div className="w-full flex flex-col items-center gap-8 animate-fade-in z-10 py-8">
      ...
      <p className="text-sm font-bold text-brand-charcoal/50 dark:text-dark-text/50 uppercase tracking-widest transition-colors">{t('rec_tap')}</p>
    </div>
  );
};